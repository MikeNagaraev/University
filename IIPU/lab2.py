    ...
    for key in device_info.keys():
        if(device_info[key]):
            print('%s: %s' % (key.capitalize().replace('_', ' '),
                              device_info[key]))
    print_modes(get_drive_modes(device))
    print('------ \n')
...
def decode_interface(word):
    modes = {0x2: 'ATA-1' , 0x4: 'ATA-2', 0x8 : 'ATA-3',
             0x10: 'ATA-5', 0x20: 'ATA-6', 0x40: 'ATA-7', 0x80: 'ATA-8' }
 
    result = ''
    for i in [0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 0x80]:
        if i & word == i:
            result += '{} '.format(modes[i])
...
def get_drive_modes(device):
    with open('/dev/' + device.sys_name, "rb") as fd:
